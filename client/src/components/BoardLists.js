...
class BoardLists extends Component {
  ...
  handleClick = (slug) => {
    this.props.dispatch(
      actions.getSingleBoard(slug, (success) => {
        if (success) {
  ...
  handleSubmit = (e, boardId) => {
    ...
    this.props.dispatch(
      actions.updateBoard(boardId, data, (success) => {
        if (success) {
          this.setState({
            isOpen: false,
          });
                ...
                boards.map((board) => {
                  return (
                    <div key={board._id} className="board-card">
                      {isOpen &&
                      openInputBox &&
                      openInputBox.includes(board._id) ? (
                        <>
                          <form
                            action=""
                            onSubmit={(e) => this.handleSubmit(e, board._id)}
                          >
                            <input
                              name="boardName"
                              type="text"
                            ...
                            />
                            <span onClick={() => this.handleClose(board._id)}>
                              x
                            </span>
                          </form>
                        </>
                        ...
                        <>
                          <span onClick={() => this.handleClick(board.slug)}>
                            {board.boardName}
                          </span>
                          <span
                            className="edit-icon"
                            onClick={() =>
                              this.handleUpdate(board._id, board.slug)
                            }
                          >
                          ...
                          <span
                            ...
                            onClick={() => this.handleDelete(board._id)}
                          >
